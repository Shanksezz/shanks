...
async def _rename(name, og=None):
    try:
        ...
        if anime_name and data.get("episode_number"):
            return (
                f"[@Izuku48] [S{data.get('anime_season') or 1}-{data.get('episode_number') or ''}] {(await get_english(anime_name))} [{data.get('video_resolution').replace('p', 'px264' if og else 'px265') or ''}].mkv".replace(
                    "‘", ""
                )
                .replace("’", "")
                .strip()
        ...
        if anime_name:
            return (
                f"[@Izuku48] {(await get_english(anime_name))} [{data.get('video_resolution').replace('p', 'px264' if og else 'px265') or ''}].mkv".replace(
                    "‘", ""
                )
                .replace("’", "")
                .strip()
