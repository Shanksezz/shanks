...
async def _start(event):
    ...
    if msg_id:
        ...
        await event.reply(
            ...
            buttons=[
                [
                    Button.url("Developer", url="t.me/Killua_zip"),
                    Button.url("My Owner", url="t.me/Killua_zip"),
                ]
            ],
        )
    await xnx.delete()
    ...
    try:
        ...
        if os.path.exists(dl):
            if compress:
                ...
                cmd = f'''{Var.FFMPEG} -i """{dl}""" -metadata "Encoded By"="t.me/Killua_zip" -preset ultrafast -c:v libx265 -crf 25 -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? """{out}""" -y'''
